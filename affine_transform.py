...
#Example on two patients - heart export
"""
#17
...
broken_heart_17 = rotate(broken_heart_17, angle=-26)   #Rotate matrix
...
output = nib.Nifti1Image(broken_heart_17, broken.affine)
nib.save(output,  os.path.join('data/segthor_train/train/Patient_17', 'broken_heart.nii.gz') )

#22
broken = nib.load('data/segthor_train/train/Patient_22/GT.nii.gz')  #loaad image to fix .nii.gz
broken_np = np.array(broken.dataobj)    # to np arraay
broken_heart_22 = np.where(broken_np == 2, broken_np, 0) # HEART - MASK 2

broken_heart_22 = rotate(broken_heart_22, angle=-26)   #Rotate matrix

#Transform broken heart
broken_heart_22 = np.roll(broken_heart_22, round(x), axis=0)
broken_heart_22 = np.roll(broken_heart_22, round(y), axis=1)
broken_heart_22 = np.roll(broken_heart_22, round(z), axis=2)

output = nib.Nifti1Image(broken_heart_22, broken.affine)
nib.save(output,  os.path.join('data/segthor_train/train/Patient_22', 'broken_heart.nii.gz') )
"""



#*******+*+*+****+  TO DO - ROUND OUTPUT TO 2 TO AVOID DIFFERENT CLASSES - AKA EVERYTHING THAT IS NOT 0 BECOMES 2*******
